...
merge_performers <- function(spek, performers_table){
  ...
  pt_list <- jsonlite::fromJSON(jsonlite::toJSON(performers_table), 
                                simplifyDataFrame = F, simplifyVector = T)
  ...
  sp_updated <- lapply(spek_performers, 
                       FUN = update_performer, 
                       annotated_performers = pt_list)
...
update_performer <- function(sp, annotated_performers){
  ...
  if (is.null(ap)) sp else mundify_list(sp, ap)
}
...
mundify_list <- function (x, val, keep.null = FALSE)
{
  ...
  for (value in values) {
    ...
    if (!value_exists_in_x) x[[length(x) + 1]] <- value 
  }
  return(x)
}
